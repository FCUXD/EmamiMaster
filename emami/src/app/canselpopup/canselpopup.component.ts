import { Component, OnInit } from '@angular/core';
import { DoctorService } from '../doctor.service';
import { Router } from '@angular/router';

@Component({
  selector: 'app-canselpopup',
  templateUrl: './canselpopup.component.html',
  styles: [`
  :host >>> .popover-body {
   padding: 0.5rem 0.75rem;
   color: #212529;
   width: 561px;
   background: #ffffff;
   z-index: 8;
   border-radius: 10px;
   box-shadow: 0px 3px 9px 0 rgba(0, 0, 0, 0.1);
 }
 :host >>> .popover{
   border:none;
  box-shadow: 0px 3px 9px 0 rgba(0, 0, 0, 0.1);
   left: 0% !important;
 }
 :host >>>.bs-popover-bottom .arrow::before, .bs-popover-auto[x-placement^="bottom"] .arrow::before {
 background:url("././assets/images/popoverArrow.png") no-repeat; 
 border: none;
 top: 1px;
 right: -273px;
 } 
 :host >>> .bs-popover-bottom{
  border:none;
  box-shadow: 0px 3px 9px 0 rgba(0, 0, 0, 0.1);
   left: 0% !important;
 }
 :host >>> .popover .arrow {
  position: absolute;
  display: block;
  width: 1rem;
  height: 0.5rem;
  margin: 0 0.3rem;
  left: 101px;
}
:host >>> .popover {
  color: #000;
  max-width: 60%;
  width: 650px;
}
 
 .modal {
   position: fixed;
   top: 0;
   right: 0;
   bottom: 0;
   left: 0;
   z-index: 1050;
   background: rgba(0, 0, 0, 0.5);
   overflow: hidden;
   outline: 0;
 } 
 .NoLeftBtn{
  width: 122px !important;
  height: 29px !important;
  padding: 0px !important; 
  background-color: transparent;
border: 1px solid #cfa15a;
outline-offset: 0px;
font-family: 'TrajanPro-Bold';
box-shadow: none;
box-shadow: none !important;

font-family: 'TrajanPro-Bold';
 }
 .GetStartedBtn{
   padding: 8px 28px;
   outline-offset: -6px;
 -moz-outline-radius: 5px;
 font-size: 12px;
 border-radius: 5px; 
 box-shadow: 0px 3px 9px 0 rgba(0, 0, 0, 0.13);
 } 
 .CanselBtnCard{
  background-color: transparent;
 border: 1px solid #cfa15a;
 outline-offset: 0px;
 font-family: 'TrajanPro-Bold';
 box-shadow: none;
 box-shadow: none !important;
 width: 152px;
height: 34px;
 padding: 8px 42px !important; 
 font-family: 'TrajanPro-Bold';
 } 
 .datepdiv{
   background-color: #d6ad66;
 border-radius: 5px;
 }
 .modal.fade.show {
   display: block;
 }
 .modal {
   position: fixed;
   top: 0;
   right: 0;
   bottom: 0;
   left: 0;
   z-index: 1050;
   background: rgba(0, 0, 0, 0.5);
   overflow: hidden;
   outline: 0;
 }
 .Modlheading{
   font-family: 'TrajanPro-Bold';
   font-size: 18px;
   margin: 0 auto;
 } 
 .modal-body p, .modal-body h4, .modal-body h5, label, .textinput {
   font-family: 'TrajanPro-Bold';
   font-size: 14px;
 }  
 .textinput{
     background-color: #f9f4e4;
     border: none;
 }
 .modal-footer {
  
   border-top: 1px solid #d6ad66;
 }
 .footerbtn{
   text-align: center;
 width: 100%;
 float: left;
 }
 .yesbtn{
   width: 122px;
   height: 33px;
 }
 .MarginCenter{
   margin: 0 auto;
 }
 .modal-header {
 
   border-bottom: 1px solid #d6ad66;
 }
 .datepdiv {
   font-family: 'PlayfairDisplay-Bold' !important;
   font-size: 20px !important;
   padding: 0px;
   width: 74px !important;
 height: 54px;
 } 
 .datepdiv span{
   font-family: 'PlayfairDisplay-Regular';
   font-size: 14px;
   width: 100%;
   margin-top: -6px;
 float: left;
 }
 .modal-body {
  
   padding: 8% 8% 2% 8%;
 } 
 .modal-dialog {
   max-width: 561px;
 
 } 
 .PopupBody p{
   width: 67%;
 float: none;
 text-align: left;
 margin: 0 auto;
 } 
 .btn-green-1{
  font-size: 12px;
  letter-spacing: 1.2px;
  border-radius: 3px;
  border: solid 1px #84b367;
  background-color: #bccf60;
  font-family: 'TrajanPro-Bold';
  box-shadow: 0 0 0 2px #bccf60;
  padding: 3px 25px;
  }
 
 `],
})
export class CanselpopupComponent implements OnInit {

  responseData;
  AppointmentId;
  constructor(private doctorService: DoctorService, private router: Router) { }
  headers = new Headers({ 'Content-Type': 'application/json' });
  ngOnInit() {
   
  }
  custom = "modal";
  popup() {
    this.custom = "show";
  }
  close() {
    this.custom = "modal";
  }
  
  CancelAppointment(text1) {

    this.AppointmentId = JSON.parse(localStorage.getItem("dataId"));
  
    var jsondata = { "id": this.AppointmentId.patientId, "reason": text1, "appointmentId": this.AppointmentId.id }
    
console.log("jsondata=="+JSON.stringify(jsondata));
    this.doctorService.CancelAppointmentDoctor(jsondata, this.headers).subscribe((data: any) => {
           this.responseData=data;
        this.router.navigate(['doctor_landingpage']);
        alert(this.responseData.MSG);
    })

  }
}
